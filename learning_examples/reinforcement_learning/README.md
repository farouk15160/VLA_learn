...
Companion to `reinforcement_learning.py`. This starts from first principles and
assumes only that you are comfortable with supervised learning.
...
In code that is exactly one line (`reinforcement_learning.py`):
...
```bash
for s in 0 1 2; do
  .venv/bin/python learning_examples/reinforcement_learning/reinforcement_learning.py --headless --updates 120 --seed $s
  .venv/bin/python learning_examples/reinforcement_learning/reinforcement_learning.py --headless --updates 120 --seed $s --no-baseline
done
```
...
**So the supervised-learning example is closer to a real VLA than this file is.**
