...
Companion to `grid_delivery_robot.py`. This section answers the two MDP-design
exercises. Read [`../reinforcement_learning/README.md`](../reinforcement_learning/README.md)
first if the words *policy gradient* are new;
...
* **Could an excessively large step penalty encourage dangerous shortcuts?**
  ...
  from 256 to 512 halved it, so the step penalty had to halve with it. The
  implemented constants preserve exactly this bound, so the
...
```bash
.venv/bin/python learning_examples/grid_delivery/grid_delivery_robot.py --headless --updates 800 --goal 127,127
```
...
to **−0.01** to keep the same ~2× margin. The reward-bound derivation shows the
relationship that must be preserved when those constants change.
